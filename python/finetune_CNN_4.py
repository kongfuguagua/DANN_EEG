...
train_dataset_finetune = GetLoader_96(data_root=os.path.join(test_path, 'bump'),
                          data_list=os.path.join(test_path, 'train.txt'),
                          transform=data_transforms)
train_loader_finetune = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
test_dataset = GetLoader_96(data_root=os.path.join(test_path, 'bump'),
                         data_list=os.path.join(test_path, 'test.txt'),
                         transform=data_transforms)
...
for epoch in range(num_epochs):
    ...
    acc_dir.append(100 * torch.true_divide(n_correct, total))
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for _, (t_img, t_label) in enumerate(test_loader):
            ...
            total += t_label.size(0)  # 计算所有的label数量
            n_correct += (predicted == t_label.squeeze(-1)).sum()  # 计算预测对的label数量
    acc_fin.append(100 * torch.true_divide(n_correct, total))
    print('Accuracy of the network on the %d test images: %d %%' % (total, (100 * torch.true_divide(n_correct, total))))
